...
import { GET_OPTHALMOLOGY_EXAMINATION_DETAIL, GET_WAITING_LIST } from "../../../config/apiConfig";
  ...
  const fetchDistanceVision = async () => {
    try {
      ...
      if (res?.status === 200 && res?.response)
        setDistanceVisionOptions(res.response);
    } catch (e) {
      console.error(e);
    }
  };
...
const handleReset = async (e) => {
  ...
  const handleSave = async (e) => {
    ...
    if (isRecallMode) {
      setPopupMessage({
        message: "Vision examination data saved locally!",
        type: "success",
        onClose: () => {
          setPopupMessage(null);
          setShowForm(false);
    ...
    if (requiredFields.some((f) => !formData[f])) {
      showPopup(
        "Please fill all required vision fields (Distance & Near for both eyes)",
        "error",
      );
      return;
    }
    ...
    try {
      ...
      const response = await postRequest(
        "/opd/saveOphthalmologyExaminationDetails",
        payload,
      );
      if (response?.status === 200) {
        showPopup("Vision examination saved successfully!", "success", async () => {
          if (!patientId && !visitId) {
            await closeForm();
          }
        });
