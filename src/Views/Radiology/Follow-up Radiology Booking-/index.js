...
useEffect(() => {
  ...
  const handlePageChange = (page) => {
    // page from Pagination component is 1-based
    setCurrentPage(page); // store as 1-based
    handleSearch(page - 1); // API is 0-based
  };
  ...
  const handleSubmit = async (shouldNavigateToPayment = false) => {
    ...
    if (isFormValid) {
      try {
        ...
        // Build investigationReq array
        formData.rows.forEach((row, index) => {
          if (row.itemId) {
            radiologyData.investigationReq.push({
              id: row.itemId,
              checkStatus: checkedRows[index] || false,
              appointmentDate:
                row.date || new Date().toISOString().split("T")[0],
              actualAmount: Math.round(
                Number.parseFloat(row.originalAmount) || 0,
              ),
              discountedAmount: Math.round(
                Number.parseFloat(row.discountAmount) || 0,
              ),
        ...
        const radiologyResult = await putRequest(
          `${UPDATE_RADIOLOGY}`,
          radiologyData,
        );
        if (!radiologyResult || radiologyResult.status !== 200) {
          throw new Error(
            radiologyResult?.message || "Radiology registration failed.",
          );
        }
          ...
          showPopup(RADIOLOGY_REG_SUCC_MSG, "success", false, () => {
            navigate("/payment", {
              state: {
                ...
                patientId,
                billingHeaderIds: radiologyResult.data.billingHeaderIds,
                radiologyData: radiologyResult,
                selectedItems: {
                  investigations: radiologyData.investigationReq.filter(
                    (i) => i.type === "i" && i.checkStatus,
                  ),
                  packages: radiologyData.investigationReq.filter(
              ...
              <div className="card-header py-3 no-bg bg-transparent d-flex align-items-center px-0 justify-content-between border-bottom flex-wrap">
                <div className="d-flex align-items-center w-100">
                  <h3 className="fw-bold mb-0">
                    Radiology Booking for Registered Patient{" "}
                  </h3>
...
export default RadiologyBookingRegisteredPatient;
