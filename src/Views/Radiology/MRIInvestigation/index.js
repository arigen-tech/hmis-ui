...
import { MRI_MODALITY, CANCEL_OR_COMPLETE_RADIOLOGY_INVESTIGATION, STATUS_X } from "../../../config/apiConfig";
    ...
    try {
      ...
      if (response?.response) {
        const mappedData = response.response.content.map(item => ({
          ...
          status: "n" // Default status since API doesn't provide it
        }));

        setMriData(mappedData);
                ...
                <div className="row mt-3">
                  <div className="col-md-12 text-end">
                    <span className="text-muted">
                      Showing {mriData.length} of {totalElements} records
                    </span>
                  </div>
                </div>
              </div>
