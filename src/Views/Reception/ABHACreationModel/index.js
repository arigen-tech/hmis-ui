...
const ABHACreationModal = ({
  ...
  const handleConsentChange = (e) => {
    ...
    setConsents((prev) => {
      ...
      const allChecked = Object.values(newConsents).every(
        (val) => val === true,
      );
  ...
  const validateTab3 = () => {
    ...
    if (!isValidAbhaAddress(cleanAddress)) {
      Swal.fire(
        "Validation Error",
        "ABHA address should contain only letters, numbers, and underscores (e.g., john_doe_123).",
        "warning",
      );
      return false;
    }
  ...
  }, [currentTab, otpData.txnId, isExistingAbha, isCreatingAbha]);

  const ABHA_ADDRESS_PATTERN = /^(?=.{3,60}$)[a-zA-Z0-9]+(?:_[a-zA-Z0-9]+)*$/;

  const isValidAbhaAddress = (value = "") => {
    if (!value || typeof value !== "string") return false;
    return ABHA_ADDRESS_PATTERN.test(value.trim());
  };

  const normalizeAbhaAddressInput = (value = "") => {
  ...
  const fetchAbhaAddressSuggestions = async () => {
    ...
    try {
      const response = await integrationService.getAbhaAddressSuggestion({
        tnxId: otpData.txnId,
      });
      ...
      console.error("Error fetching ABHA address suggestions:", error);
      setSuggestions([]);
    } finally {
  ...
  // Handle ABHA address change
  const handleAbhaAddressChange = (value) => {
    const normalized = normalizeAbhaAddressInput(value);
    setAbhaAddress(normalized);

    // Validate on change
    if (normalized && !isValidAbhaAddress(normalized)) {
      setAddressError(
        "Enter a valid ABHA ID / Address. Spaces are not allowed. Use letters, numbers, and underscores only.",
      );
    } else {
      setAddressError("");
    }
...
const findGenderId = (genderName) => {
  ...
  let found = genderData.find(
    (g) => g.genderName?.toLowerCase() === normalizedGender
  );
  
  // If not found, try includes match
  if (!found) {
    found = genderData.find(
      (g) => g.genderName?.toLowerCase().includes(normalizedGender) ||
  ...
  if (!found) {
    ...
    for (const [key, values] of Object.entries(commonMappings)) {
      if (values.some(v => normalizedGender.includes(v) || v.includes(normalizedGender))) {
        found = genderData.find(
          (g) => g.genderName?.toLowerCase() === key
        );
        if (found) break;
      }
    }
...
const fetchAbhaDetails = async (xtoken, isType) => {
  ...
  try {
    ...
    if (response?.status === false) {
      console.error("Failed to fetch ABHA details:", response?.message);
      return null;
    }
    ...
    return {
      ...
      stateName: profile.stateName || profile.state || "",
      stateId: findStateId(profile.stateName || profile.state),
      districtName: profile.districtName || profile.district || "",
      districtId: findDistrictId(
        profile.districtName || profile.district,
        findStateId(profile.stateName || profile.state)
      ),
...
const handleVerifyOtp = async () => {
  ...
  try {
    ...
    const isNew = response?.response?.isNew;
    const isExisting = abhaProfile || isNew === false;

    let fullProfile = null;
    if (xtoken) {
      fullProfile = await fetchAbhaDetails(xtoken, isType);
    }

    if (isExisting) {
      if (!fullProfile && abhaProfile) {
        const directProfile = response?.response || {};
        fullProfile = {
          abhaNumber: directProfile.ABHANumber || directProfile.healthIdNumber || "",
          abhaAddress: directProfile.preferredAbhaAddress || directProfile.healthId || "",
          name: directProfile.name || aadhaarData.consentName,
          gender: directProfile.gender || "",
          genderId: findGenderId(directProfile.gender),
          mobileNumber: directProfile.mobile || directProfile.mobileNumber || otpData.mobileNumber,
          dateOfBirth: directProfile.dayOfBirth || directProfile.dateOfBirth || "",
          stateName: directProfile.stateName || "",
          stateId: findStateId(directProfile.stateName),
          districtName: directProfile.districtName || "",
          districtId: findDistrictId(
            directProfile.districtName,
            findStateId(directProfile.stateName)
          ),
          address: directProfile.address || "",
          pincode: directProfile.pincode || "",
          photo: directProfile.profilePhoto || "",
          xtoken: xtoken,
          isType: isType,
          fullProfile: directProfile,
        };
      }
      ...
      if (fullProfile) {
        ...
        Swal.fire({
          ...
          <div>
            ...
            <p><strong>ABHA Number:</strong> ${formatAbhaNumber(fullProfile.abhaNumber || "Not found")}</p>
            <p><strong>ABHA Address:</strong> ${fullProfile.abhaAddress || "Not set"}</p>
            <p><strong>Name:</strong> ${fullProfile.name || aadhaarData.consentName}</p>
          </div>
        `,
        ...
        throw new Error("Unable to fetch ABHA profile details.");
      }
    ...
    if (xtoken) {
      setOtpData((prev) => ({ ...prev, xtoken, isType }));
    }
    ...
    Swal.fire(
      "Verified",
      "OTP verified successfully! You can now create your ABHA address.",
      "success",
    );
  } catch (error) {
    setLoading((prev) => ({ ...prev, verifyOtp: false }));
  ...
  const handleCreateAbha = async () => {
    ...
    try {
      ...
      const response = await integrationService.updateAbhaAddress({
        tnxId: otpData.txnId,
      ...
      const abhaNumberResponse =
        fullProfile?.abhaNumber ||
        response?.response?.abhaNumber ||
      ...
      Swal.fire({
        ...
        <div>
          ...
          <p><strong>ABHA Address:</strong> ${finalAbhaAddress}</p>
          <p><strong>Name:</strong> ${fullProfile?.name || aadhaarData.consentName}</p>
          ${fullProfile?.gender ? `<p><strong>Gender:</strong> ${fullProfile.gender}</p>` : ""}
          ${fullProfile?.dateOfBirth ? `<p><strong>Date of Birth:</strong> ${fullProfile.dateOfBirth}</p>` : ""}
          ${fullProfile?.mobileNumber ? `<p><strong>Mobile:</strong> ${fullProfile.mobileNumber}</p>` : ""}
        </div>
      `,
        ...
        onSuccess?.({
          ...
          abhaAddress: finalAbhaAddress,
          consentName: fullProfile?.name || aadhaarData.consentName,
          mobileNumber:
            fullProfile?.mobileNumber ||
            otpData.mobileNumber ||
            aadhaarData.mobileNumber,
          isNewAbha: true,
          gender: fullProfile?.gender || "",
          genderId: fullProfile?.genderId || null,
          stateName: fullProfile?.stateName || "",
          stateId: fullProfile?.stateId || null,
          districtName: fullProfile?.districtName || "",
          districtId: fullProfile?.districtId || null,
          dateOfBirth: fullProfile?.dateOfBirth || "",
          address: fullProfile?.address || "",
          pincode: fullProfile?.pincode || "",
          photo: fullProfile?.photo || "",
          xtoken: otpData.xtoken || "",
          isType: otpData.isType || "",
      ...
      Swal.fire(
        "Error",
        error.message || "Unable to create ABHA account.",
        "error",
      );
    }
  };
      ...
      <div className="abha-card-container">
        <div className="alert alert-info mb-3">
          ...
          <p className="mb-0 mt-1">
            This Aadhaar number is already linked to an ABHA. You can view the
            profile below.
          </p>
        </div>
                ...
                <div className="row g-2">
                  ...
                  {profile.stateName && (
                    <div className="col-6">
                      ...
                      <p className="fw-bold">
                        {profile.stateName}{" "}
                        {profile.stateId ? `(ID: ${profile.stateId})` : ""}
                      </p>
                    </div>
                  )}
          ...
          <div className="modal-body p-4">
              ...
              <div className="d-flex justify-content-between mb-4 position-relative">
                ...
                {[1, 2, 3].map((step) => (
                  ...
                  >
                    ...
                    <span
                      className={`small ${currentTab >= step ? "text-dark fw-bold" : "text-muted"}`}
                    >
                      {step === 1
                        ? "Aadhaar"
                        : step === 2
              ...
              <div className="tab-content">
                ...
                <div className="mt-4">
                  <h6 className="fw-bold mb-3">
                    Consent Declarations <span className="text-danger">*</span>
                  </h6>
                  ...
                  >
                    <div className="form-check mb-2">
                      ...
                      <label className="form-check-label" htmlFor="consent1">
                        ...
                        ("UIDAI"), and my demographic information for the
                        purpose of creating an Ayushman Bharat Health Account
                        number ("ABHA number") and Ayushman Bharat Health
                        Account address ("ABHA Address"). I authorize NHA to use
                        my Aadhaar number / Virtual ID for performing Aadhaar
                        based authentication with UIDAI as per the provisions of
                    ...
                    <div className="form-check mb-2">
                      ...
                      <label className="form-check-label" htmlFor="consent2">
                        I consent to usage of my ABHA address and ABHA number
                        for linking of my legacy (past) government health
                    ...
                    <div className="form-check mb-2">
                      ...
                      <label className="form-check-label" htmlFor="consent5">
                        I, {aadhaarData.consentName || "[Name]"}, confirm that I
                        have duly informed and explained the beneficiary of the
                    ...
                    <div className="form-check mb-2">
                      ...
                      <label className="form-check-label" htmlFor="consent6">
                        I, {aadhaarData.consentName || "[Name]"}, have been
                    ...
                    <div className="form-check mb-2">
                      ...
                      <label className="form-check-label" htmlFor="consent7">
                        I understand that I can manage my ABHA through the ABDM
                        portal and can withdraw my consent at any time.
                      </label>
                    </div>
                  </div>
                </div>
            ...
            {currentTab === 2 && !isExistingAbha && (
              <div className="tab-content">
                ...
                <div className="mb-3">
                  <label className="form-label fw-bold">
                    Mobile Number (Aadhaar Registered){" "}
                    <span className="text-danger">*</span>
                  </label>
                  <input
                    type="text"
              ...
              <div className="tab-content">
                {isExistingAbha && existingAbhaProfile ? (
                  ...
                  <>
                    ...
                    <div className="mb-3">
                      <label className="form-label fw-bold">
                        ABHA Address <span className="text-danger">*</span>
                      </label>
                      <input
                        type="text"
                        className={`form-control ${addressError ? "is-invalid" : abhaAddress && isValidAbhaAddress(abhaAddress) ? "is-valid" : ""}`}
                        value={abhaAddress}
                        onChange={(e) =>
                          handleAbhaAddressChange(e.target.value)
                        }
                        placeholder="Enter preferred ABHA ID / Address (e.g., username)"
                      />
                      {addressError ? (
                        ...
                        <small className="text-muted">
                          Use letters, numbers, and underscores only (e.g.,
                          john_doe_123)
                        </small>
                      )}
                    </div>
                    ...
                    {suggestions.length > 0 && (
                      <div className="mb-3">
                        ...
                        <div className="d-flex flex-wrap gap-2">
                          {suggestions.map((suggestion) => (
                            <button
                              key={suggestion}
                              type="button"
                              className={`btn ${abhaAddress === suggestion ? "btn-primary" : "btn-outline-primary"} btn-sm`}
                        ...
                        <small className="text-muted">
                          Click on a suggestion to use it, or type your own
                          address above.
                        </small>
                      </div>
                    )}
