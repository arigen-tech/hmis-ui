...
const OTPriorityMaster = () => {
  const [formData, setFormData] = useState({ priorityName: "" });
  ...
  const PRIORITY_NAME_MAX_LENGTH = 50;

  const dummyData = [
    { id: 1, priorityName: "Elective", status: "Y" },
    { id: 2, priorityName: "Urgent", status: "Y" },
    { id: 3, priorityName: "Emergency", status: "N" }
  ];
  ...
  useEffect(() => {
    setIsFormValid(formData.priorityName.trim() !== "");
  }, [formData]);

  const filteredData = data.filter(item =>
    item.priorityName?.toLowerCase().includes(searchQuery.toLowerCase())
  );

  const handlePageChange = (page) => setCurrentPage(page);
  const handleEdit = (item) => {
    setEditingItem(item);
    setFormData({ priorityName: item.priorityName || "" });
    setShowForm(true);
  };
  const handleSave = (e) => {
    e.preventDefault();
  ...
  const handleConfirm = (confirmed) => {
    if (confirmed && confirmDialog.id !== null) {
      ...
      setTimeout(() => {
        setProcess(false);
        showPopup(`Priority ${confirmDialog.newStatus === "y" ? "activated" : "deactivated"}`, "success", () => {
          setData(dummyData);
          setCurrentPage(1);
        });
      }, 500);
  ...
  const handleInputChange = (e) => {
    setFormData({ priorityName: e.target.value });
  };
  ...
  const handleActivate = () => {
    if (editingItem && editingItem.status?.toLowerCase() === "n") {
      ...
      setTimeout(() => {
        setProcess(false);
        showPopup("Priority activated", "success", () => { setData(dummyData); resetForm(); });
      }, 500);
    }
  };
        ...
        <div className="col-12 grid-margin stretch-card">
          <div className="card form-card">
            <div className="card-header d-flex justify-content-between align-items-center">
              <h4 className="card-title p-2">OT Priority Master</h4>
              <div className="d-flex gap-2">
                {!showForm ? (
                  <>
                    <form className="d-inline-block searchform me-2">
                      <div className="input-group searchinput">
                        <input type="search" className="form-control" placeholder="Search priority..." value={searchQuery} onChange={handleSearchChange} />
                        <span className="input-group-text"><i className="fa fa-search"></i></span>
                      </div>
                    </form>
                    <button className="btn btn-success" onClick={handleRefresh}><i className="mdi mdi-refresh"></i> Show All</button>
                    <button className="btn btn-success" onClick={() => { setEditingItem(null); setFormData({ priorityName: "" }); setShowForm(true); }}><i className="mdi mdi-plus"></i> Add</button>
                  </>
                ) : (
                ...
                <>
                  <div className="table-responsive">
                    <table className="table table-bordered table-hover align-middle">
                      <thead className="table-light"><tr><th>Priority Name</th><th>Status</th><th>Edit</th></tr></thead>
                      <tbody>
                          ...
                          <tr key={item.id}>
                            ...
                            <td>
                              <div className="form-check form-switch">
                                <input className="form-check-input" type="checkbox" checked={item.status === "Y"} onChange={() => handleSwitchChange(item.id, item.priorityName, item.status === "Y" ? "n" : "y")} />
                ...
                <form onSubmit={handleSave}>
                  <div className="row">
                    <div className="form-group col-md-6">
                      <label>Priority Name <span className="text-danger">*</span></label>
                      <input type="text" className="form-control" placeholder="Enter priority name" value={formData.priorityName} onChange={handleInputChange} maxLength={PRIORITY_NAME_MAX_LENGTH} required disabled={process} />
                    </div>
                  </div>
...
export default OTPriorityMaster;