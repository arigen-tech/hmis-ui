...
const Hospitalmaster = () => {
    ...
    if (editingHospital) {
      setHospitalData(
        hospitalData.map((hospital) =>
          hospital.code === editingHospital.code
            ? {
                ...hospital,
  ...
  const handleCreateFormSubmit = (e) => {
    ...
    if (formData.hospitalName) {
      ...
      const newCode = formData.code || `H${(hospitalData.length + 1).toString().padStart(3, "0")}`
      setHospitalData([...hospitalData, { ...formData, code: newCode, status: "y" }])
  ...
  const handleSwitchChange = (code, newStatus) => {
    setConfirmDialog({ isOpen: true, hospitalId: code, newStatus })
  }
    ...
    if (confirmed && confirmDialog.hospitalId !== null) {
      setHospitalData((prevData) =>
        prevData.map((hospital) =>
          hospital.code === confirmDialog.hospitalId ? { ...hospital, status: confirmDialog.newStatus } : hospital,
        ),
      )
    }
  ...
  const currentItems = filteredHospitals.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage)
                          ...
                          <td>
                            <div className="form-check form-switch">
                              <input
                                ...
                                onChange={() => handleSwitchChange(hospital.code, hospital.status === "y" ? "n" : "y")}
                                id={`switch-${hospital.code}`}
                              />
                              <label className="form-check-label px-0" htmlFor={`switch-${hospital.code}`}>
                    ...
                    <div className="row g-3 align-items-center">
                      ...
                      <div className="col-md-6">
                        ...
                        <input
                          ...
                          className="form-control"
                          id="name"
                          value={formData.hospitalName ? formData.hospitalName : ""}
                      ...
                      <div className="col-md-6">
                        <label htmlFor="appointmentcost" className="form-label">
                          Appointment Cost
                        </label>
                        <select
                          className="form-control"
                      ...
                      <div className="col-md-6">
                        <label htmlFor="preconsultationcost" className="form-label">
                          Pre Consultation
                        </label>
                        <select
                          className="form-control mt-1"
                          id="preconsultationcost"
                          value={formData.preConsultation}
                          onChange={handleInputChange}
                          required
                        >
                          <option value="" disabled>
                    ...
                    <div className="modal-content">
                      ...
                      <div className="modal-body">
                        <p>
                          ...
                          <strong>
                            {hospitalData.find((hospital) => hospital.code === confirmDialog.hospitalId)?.name}
                          </strong>
                          ?
                        </p>
                      </div>
