...
const SterilisationMaster = () => {
  ...
  const [formData, setFormData] = useState({ sterilisationType: "" });
  ...
  const filteredData = data.filter((rec) =>
    (rec?.sterilisationType ?? "").toLowerCase().includes(searchQuery.toLowerCase()) // Fixed
  );
  ...
  const handleSave = async (e) => {
    ...
    const newValue = formData.sterilisationType.trim().toLowerCase();
    const duplicate = data.find(
      (rec) =>
        rec.sterilisationType?.trim().toLowerCase() === newValue &&
    ...
    try {
      if (editingRecord) {
        await putRequest(`${MAS_STERILISATION}/update/${editingRecord.id}`, {
          ...editingRecord,
  ...
  const handleEdit = (rec) => {
    ...
    setFormData({ sterilisationType: rec.sterilisationType || "" });
    setShowForm(true);
    setIsFormValid(true);
  };
