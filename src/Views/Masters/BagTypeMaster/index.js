...
const BagTypeMaster = () => {
    const [formData, setFormData] = useState({bagTypeName: "", description: ""});
    ...
    const [bagTypeData, setBagTypeData] = useState([
        { id: 1, bagTypeName: "Paper Bag", description: "Eco-friendly paper bags", status: "y" },
        { id: 2, bagTypeName: "Plastic Bag", description: "Regular plastic bags", status: "y" },
        { id: 3, bagTypeName: "Cloth Bag", description: "Reusable cloth bags", status: "y" },
        { id: 4, bagTypeName: "Jute Bag", description: "Natural jute bags", status: "y" },
        { id: 5, bagTypeName: "Non-Woven Bag", description: "PP non-woven bags", status: "y" },
        { id: 6, bagTypeName: "Ziplock Bag", description: "Resealable plastic bags", status: "y" },
    ]);

    const [confirmDialog, setConfirmDialog] = useState({ isOpen: false, bagTypeId: null, newStatus: false });


    const filteredBagTypes = bagTypeData.filter(bagType =>
    ...
    const handleEdit = (bagType) => {
        ...
        setFormData({bagTypeName: bagType.bagTypeName, description: bagType.description || "" });
        setShowForm(true);
    };
    ...
    const handleSave = (e) => {
        ...
        if (editingBagType) {
            setBagTypeData(bagTypeData.map(bagType =>
                bagType.id === editingBagType.id
                    ? { ...bagType, bagTypeName: updatedBagTypeName, description: updatedDescription }
                    : bagType
            ));
        } else {
            const newBagType = {
    ...
    const handleInputChange = (e) => {
        ...
        const isBagTypeNameValid = id === "bagTypeName" ? value.trim() !== "" : formData.bagTypeName.trim() !== "";
        const isDescriptionValid = true; 

        setIsFormValid(isBagTypeNameValid && isDescriptionValid);
    };

    const handleCreateFormSubmit = (e) => {
        e.preventDefault()
        if (formData.bagTypeName) {
            setBagTypeData([...bagTypeData, { ...formData, id: Date.now(), status: "y" }])
            setFormData({ bagTypeName: "", description: "" })
            setShowForm(false)
        } else {
        ...
        if (confirmed && confirmDialog.bagTypeId !== null) {
            setBagTypeData((prevData) =>
                prevData.map((bagType) =>
                    bagType.id === confirmDialog.bagTypeId ? { ...bagType, status: confirmDialog.newStatus } : bagType
                )
            );
        }
                            ...
                            {!showForm ? (
                                                    ...
                                                    <td>
                                                        <div className="form-check form-switch">
                                                            <input
                                                                ...
                                                                onChange={() => handleSwitchChange(bagType.id, bagType.status === "y" ? "n" : "y")}
                                ...
                                <form className="forms row" onSubmit={handleSave}>
                                    <div className="form-group col-md-4">
                                        <label>Bag Type Name <span className="text-danger">*</span></label>
                                        <input
                                            type="text"
                                            className="form-control"
                                            id="bagTypeName"
                                ...
                                <div className="modal d-block" tabIndex="-1" role="dialog">
                                    <div className="modal-dialog" role="document">
                                        <div className="modal-content">
                                            ...
                                            <div className="modal-body">
                                                <p>
                                                    Are you sure you want to {confirmDialog.newStatus === "y" ? 'activate' : 'deactivate'} <strong>{bagTypeData.find(bagType => bagType.id === confirmDialog.bagTypeId)?.bagTypeName}</strong>?
                                                </p>
                                            </div>
                                            <div className="modal-footer">
