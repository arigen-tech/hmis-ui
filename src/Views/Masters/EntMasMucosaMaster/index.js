...
const EntMasMucosaMaster = () => {
  ...
  const [formData, setFormData] = useState({ mucosaStatus: "" });
  ...
  const filteredData = data.filter((rec) =>
    (rec?.mucosaStatus ?? "").toLowerCase().includes(searchQuery.toLowerCase())
  );
  ...
  const handleSave = async (e) => {
    ...
    const newValue = formData.mucosaStatus.trim().toLowerCase();
    const duplicate = data.find(
      (rec) =>
        rec.mucosaStatus?.trim().toLowerCase() === newValue &&
    ...
    try {
      if (editingRecord) {
        await putRequest(`${MAS_ENT_MUCOSA}/update/${editingRecord.id}`, {
          ...editingRecord,
  ...
  const handleEdit = (rec) => {
    ...
    setFormData({ mucosaStatus: rec.mucosaStatus || "" });
    setIsFormValid(true);
    setShowForm(true);
  };
          ...
          {!showForm ? (
            ...
            <form className="row" onSubmit={handleSave}>
              <div className="form-group col-md-6">
                <label htmlFor="mucosaStatus">Mucosa Status</label>
                <input
                  type="text"
                  id="mucosaStatus"
                  className="form-control mt-1"
                  value={formData.mucosaStatus}
