...
import {  MAS_SUB_CHARGE_CODE_DROPDOWN_END_URL, REQUEST_PARAM_HOSPITAL_ID, REQUEST_PARAM_FROM_DATE, REQUEST_PARAM_TO_DATE, REQUEST_PARAM_PAGE, REQUEST_PARAM_SIZE, REQUEST_PARAM_SUB_CHARGE_CODE_ID, REJECTED_INVESTIGATIONS_END_URL, REJECTED_INVESTIGATIONS_REPORT_URL, REQUEST_PARAM_FLAG, STATUS_D, STATUS_P } from "../../../../config/apiConfig";
import {formatDateForDisplay} from "../../../../utils/dateUtils";
import { 
  FETCH_SAMPLE_REJECT_ERR_MSG, 
...
const SampleRejectionReport = () => {
    ...
    const handleToDateChange = (e) => {
        ...
        if (selectedDate > today) {
            showPopup(FUTURE_DATE_PICK_WARN_MSG, "error");
            setToDate(today);
            return;
        }
    ...
    const fetchModalityOptions = async () => {
        try {
            ...
            if (response && response.response) {
                // Filter for mainChargeId === 12 (Lab investigations)
    ...
    // Fetch sample rejection report with pagination
    const fetchSampleRejectionReport = async (page = 1, isSearchAction = false) => {
        try {
            if (isSearchAction) {
                setIsSearching(true);
            }
            ...
            const response = await getRequest(`${REJECTED_INVESTIGATIONS_END_URL}?${params.toString()}`);
            ...
            if (response && response.response) {
                ...
                const mappedData = content.map(item => ({
                    ...
                    rejectionRemarks: item.rejectionReason 
                }));
            ...
            console.error("Error fetching sample rejection report:", error);
            showPopup(FETCH_SAMPLE_REJECT_ERR_MSG, "error");
            setReportData([]);
            setTotalElements(0);
            setShowReport(true);
        } finally {
    ...
    const handlePageChange = (page) => {
        setCurrentPage(page);
        fetchSampleRejectionReport(page, false);
    };
    ...
    const handleSearch = () => {
        ...
        setCurrentPage(1);
        fetchSampleRejectionReport(1, true);
    };
    ...
    const generatePdfReport = async (flag = STATUS_D) => {
        ...
        try {
            ...
            const url = `${REJECTED_INVESTIGATIONS_REPORT_URL}?${params.toString()}`;
        ...
        <div className="content-wrapper">
            ...
            {pdfUrl && (
                <PdfViewer
                    ...
                    name={`Sample Rejection Report - ${formatDateForDisplay(fromDate)} to ${formatDateForDisplay(toDate)}`}
                />
            )}
